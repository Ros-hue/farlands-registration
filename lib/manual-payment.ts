...
export function normalizeImageMimeType(mime?: string | null): string | undefined {
...
export async function readPaymentImage(value: FormDataEntryValue | null) {
  ...
  const clientMime = normalizeImageMimeType(value.type);
  if (!image || (clientMime && clientMime !== "application/octet-stream" && clientMime !== image.mimeType)) {
    throw new HttpError(400, "The uploaded file is not a valid PNG, JPEG, or WebP image.");
  }
  return { bytes, image };
}
