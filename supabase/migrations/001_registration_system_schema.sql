-- ============================================================================
-- FARLANDS HACKATHON: REGISTRATION, PAYMENT VERIFICATION & ADMIN SYSTEM
-- Standalone PostgreSQL Database Schema for Dedicated Supabase Project
-- ============================================================================

-- 1. ENUMS & EXTENSIONS
create extension if not exists "pgcrypto";

create type public.participant_status as enum ('active', 'disabled');
create type public.registration_status as enum ('pending_payment', 'payment_processing', 'confirmed');
...
create table public.teams (
  ...
  team_name text not null check (char_length(btrim(team_name)) between 3 and 50),
  status text not null default 'active' check (status in ('active', 'disabled')),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

create unique index teams_team_id_uniq on public.teams (team_id);
create unique index teams_team_name_ci_uniq on public.teams (lower(team_name));
create index teams_team_name_idx on public.teams (team_name);
...
create table public.participants (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint participants_phone_format check (phone is null or phone ~ '^\+?[0-9]{10,15}$')
);

create unique index participants_participant_id_uniq on public.participants (participant_id);
create unique index participants_email_ci_uniq on public.participants (lower(email));
create unique index participants_auth_user_id_not_null_uniq on public.participants (auth_user_id) where auth_user_id is not null;
...
create index participants_name_auth_idx on public.participants (lower(name), auth_user_id) where auth_user_id is not null;

-- 4. AUTH PROFILES (Role Mapping)
create table public.auth_profiles (
  ...
  created_at timestamptz not null default now(),
  check (
    (role = 'participant' and participant_id is not null and username is null) or
    (role = 'admin' and participant_id is null and username is not null)
  )
);

create index auth_profiles_role_idx on public.auth_profiles (role);
...
create index registrations_team_id_idx on public.registrations (team_id);
...
-- 6. PAYMENT PROOFS (Manual UPI Verification)
...
create table public.audit_logs (
  ...
  actor_role text not null check (actor_role in ('participant', 'admin', 'system')),
  actor_id uuid,
...
create index audit_logs_actor_idx on public.audit_logs (actor_role, actor_id);
create index audit_logs_registration_id_idx on public.audit_logs (registration_id);
create index audit_logs_payment_proof_id_idx on public.audit_logs (payment_proof_id);

-- 8. DISTRIBUTED RATE LIMITS
...
-- Default Deny on all tables for anon and authenticated
revoke all on public.teams, public.participants, public.auth_profiles, public.registrations, public.payment_proofs, public.audit_logs, public.payment_rate_limits from anon, authenticated;

-- Read policies for authenticated participants on their own profile
create policy auth_profiles_own_read on public.auth_profiles
...
create policy payment_proofs_service_access on storage.objects
...
-- A. Validate Payment Proof Trigger
...
-- C. Review Manual Payment Proof (Approve / Reject)
...
-- D. Distributed Rate Limit Counter
